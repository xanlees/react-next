...
const BasicTailwind = () => {
  ...
  // useStepper
  const { state, nextStep, prevStep, progressProps, stepsProps, stepperProps } = useStepper({
    steps,
  });
  ...
  return (
    <>
      ...
      <div className="">
      <button className="bg-blue-400  hover:bg-blue-700 mx-1 text-white font-bold py-2 px-4 rounded" onClick={prevStep} disabled={!state.hasPreviousStep}>
        Prev
      </button>
      <button className="bg-blue-400  hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={nextStep}>Next</button>
      <div {...progressProps} />
      </div>
    </>
  );
