...
const index = ({ url, method }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (data && data.id) {
      dispath(handleSetLotteryID(data.id));
    }
    // if (group === "") {
    ...
    // } else {
    //   const { data } = await postAPI(method, url, sentdata);
    //   console.log(data);
          ...
          <div className="mx-5">
            ...
            <div className="grid gap-6 mb-6 lg:grid-cols-2">
              <div>
                ...
                >
                  Lottery Name
                  {/* {JSON.stringify(Lottery.lottery_id)}
                  {JSON.stringify(Lottery.lotterry_items)} */}
                </label>
                <input
                  type="text"
                  id="lottery_name"
